...
web/
  app.py                Flask app. /api/chat builds system prompt from DB events + facts + state.
...
- **Morning digest (6am):** `digest.py` queries 7-day events + IXL state + facts → LiteLLM synthesizes → ntfy.sh push.
...
| File | Contents |
|------|----------|
| `school.db` | SQLite: `events` table (157+ calendar events + email-extracted events) |
